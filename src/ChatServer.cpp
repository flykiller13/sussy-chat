...
void ChatServer::handle_new_connection(vector<pollfd> &pfds) {
  ...
  int new_fd = new_socket.get_socket_fd();
  // since the ownership of the socket moves, we need to store the fd
  clients_.push_back(move(new_socket));
  add_to_pfds(pfds, new_fd);
}

void ChatServer::handle_client_messages(vector<pollfd> &pfds,
                                        pollfd client_pfd) {
  try {
    for (Socket &client : clients_) {
      if (client.get_socket_fd() == client_pfd.fd) {
        ...
        if (client_name == client_names_.end())
        // Is the client's name registered?
        {
